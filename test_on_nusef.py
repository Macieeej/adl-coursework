...
mit_data_dir = os.path.abspath(os.path.join(current_dir, '..', 'NUSEF_data'))
fix_map_data_dir = os.path.abspath(os.path.join(current_dir, '..', 'ALLFIXATIONMAPS_NUSEF'))
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            for i in range(len(self.test_dataset.dataset)):
                ...
                print(self.test_dataset.dataset[i]['file'])
...
def main(args):
    torch.cuda.empty_cache()
    test_dataset = MIT(dataset_path=os.path.join(mit_data_dir, 'nusef_test_data.pth.tar'))
    #test_dataset = MIT(dataset_path=os.path.join(mit_data_dir, 'nusef_all_data.pth.tar'))
